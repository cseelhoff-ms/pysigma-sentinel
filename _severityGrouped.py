...
for sigmaRule in sigmaRules:
    ...
    query = 'let ' + ruleid + '=' + convertedStrings[0]
...
ruleid = 'ec570e53-4c76-45a9-804d-dc3f355ff7a7'
newTemplate: str = templateContents.replace('---RULEID---', ruleid).replace('---TITLE---', '"Sigma Windows Process Creation Critical"').replace('---DESCRIPTION---', '"Sigma Windows Process Creation Critical"').replace('---LEVEL---', 'high').replace('---QUERY---', combinedQuery).replace('---TACTICS---', tacticsString).replace('---TECHNIQUES---', techniquesString)#.replace('---TAGS---', tagsString)
#write newTemplate to file
with open('./output/Sigma Windows Process Creation Critical.json', 'w') as f:
    f.write(newTemplate)
