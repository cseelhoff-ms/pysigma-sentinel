...
for sigmaRule in sigmaRules:
    ...
    newTemplate: str = templateContents.replace('---RULEID---', ruleid).replace('---TITLE---', title).replace('---DESCRIPTION---', description).replace('---LEVEL---', level).replace('---QUERY---', query).replace('---TACTICS---', tacticsString).replace('---TECHNIQUES---', techniquesString)#.replace('---TAGS---', tagsString)
