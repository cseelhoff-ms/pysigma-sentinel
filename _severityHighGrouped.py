...
for sigmaRule in sigmaRules:
    ...
    if tacticFilter not in tactics:
        continue

    convertedStrings: list[str] = backend.convert_rule(sigmaRule)
    query = 'let ' + queryIndexString + '=' + convertedStrings[0]
 
    query += '|extend RuleId="' + ruleid + '"'
...
ruleid = 'ec570e53-4c76-45a9-804d-dc3f355ff7a8'
newTemplate: str = templateContents.replace('---RULEID---', ruleid).replace('---TITLE---', '"Sigma Windows Process Creation High ' + tacticFilter + '"').replace('---DESCRIPTION---', '"Sigma Windows Process Creation High ' + tacticFilter + '"').replace('---LEVEL---', 'medium').replace('---QUERY---', combinedQuery).replace('---TACTICS---', tacticsString).replace('---TECHNIQUES---', techniquesString)#.replace('---TAGS---', tagsString)
#write newTemplate to file
with open('./output/Sigma Windows Process Creation High ' + tacticFilter + '.json', 'w') as f:
    f.write(newTemplate)
