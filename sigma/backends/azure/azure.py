...
from sigma.conditions import ConditionItem, ConditionAND, ConditionOR, ConditionNOT, ConditionType, ConditionFieldEqualsValueExpression
from sigma.types import SigmaCompareExpression, SigmaRegularExpressionFlag, SpecialChars
import re
...
class AzureBackend(TextQueryBackend):
    ...
    group_expression: ClassVar[Optional[str]] = "({expr})"   # Expression for precedence override grouping as format string with {expr} placeholder
    ...
    eq_token: ClassVar[Optional[str]] = token_separator + "=~" + token_separator  # Token inserted between field and value (without separator)
